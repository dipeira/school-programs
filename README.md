...
- **Είσοδος μέσω ΠΣΔ:** Ταυτοποίηση χρηστών μέσω του SSO του Πανελληνίου Σχολικού Δικτύου.
- **Ψηφιακή Υποβολή:** Πλήρης φόρμα καταχώρησης προγράμματος με καρτέλες (Σχολεία, Εκπαιδευτικοί, Πρόγραμμα, Πρόοδος).
- **Αυτόματες Βεβαιώσεις σε PDF:** Άμεση λήψη επίσημων βεβαιώσεων σε μορφή PDF (με αυτόματη μετατροπή από το πρότυπο `.docx`) μετά την έγκριση.
- **Δημοσίευση στον Κατάλογο:** Επιλογή για προβολή του προγράμματος στον δημόσιο κατάλογο, κείμενο παρουσίασης και άμεση μεταφόρτωση έως 8 φωτογραφιών (έως 8MB συνολικά).
...
- **Μαζική Λήψη Βεβαιώσεων σε PDF:** Αυτόματη ταχεία (batch) παραγωγή και μετατροπή όλων των εγκεκριμένων βεβαιώσεων σε PDF και λήψη τους σε συμπιεσμένο αρχείο (.zip) με εγγυημένο αυτόματο καθαρισμό προσωρινών αρχείων.
- **Εξαγωγή Δεδομένων:** Μαζική εξαγωγή στοιχείων σε αρχείο Excel.
- **Διαχείριση Καταλόγου:** Έγκριση, διόρθωση και διαχείριση των δημοσιευμένων προγραμμάτων και των εικόνων τους.

### Δημόσιος Κατάλογος Προγραμμάτων (Public Catalog)
- **Διαδραστική Αναζήτηση:** Αναζήτηση προγραμμάτων με βάση το όνομα του σχολείου ή τον τίτλο με ελληνικό, accent-insensitive φιλτράρισμα.
- **Σελιδοποίηση (Pagination):** Καθαρή παρουσίαση των αποτελεσμάτων σε σελίδες των 9 καρτών.
- **Zoom Lightbox Gallery:** Προβολή φωτογραφιών του προγράμματος σε πλήρη ανάλυση με διαδραστικό overlay.
...
- **Integrations & Utilities:** phpCAS (SSO), PHPOffice/PHPWord (Document Generation), LibreOffice (Headless DOCX to PDF Conversion)
...
### 2. Εγκατάσταση LibreOffice (Για Μετατροπή Βεβαιώσεων σε PDF)
Η εφαρμογή χρησιμοποιεί το LibreOffice σε headless mode για την αυτόματη μετατροπή των βεβαιώσεων από Word (`.docx`) σε PDF με πλήρη διατήρηση της μορφοποίησης:
* **Ubuntu Server:** 
  ```bash
  sudo apt update && sudo apt install -y libreoffice
  ```
* **Windows / XAMPP:** 
  Κατεβάστε και εγκαταστήστε το LibreOffice (στην προεπιλεγμένη διαδρομή `C:\Program Files\LibreOffice\program\soffice.exe`).

### 3. Εγκατάσταση Εξαρτήσεων (Composer)
...
### 5. Ρύθμιση Σύνδεσης και Εφαρμογής (conf.php)
...
### 7. SSO & CAS (ΠΣΔ)
Η εφαρμογή είναι προ-ρυθμισμένη για χρήση με το **sso.sch.gr**.
* **Αυτόματος εντοπισμός URL:** Η εφαρμογή ανιχνεύει αυτόματα τη διεύθυνση URL επιστροφής (service URL) του server σας (υποστηρίζοντας και reverse proxies / SSL-termination).
* **Χειροκίνητος ορισμός URL:** Αν απαιτείται, μπορείτε να ορίσετε σταθερά τη διεύθυνση URL της εφαρμογής σας μέσω της μεταβλητής `$prCasServiceUrl` στο `conf.php`:
  ```php
  $prCasServiceUrl = 'https://srv1-dipe.ira.sch.gr'; // αφήστε κενό ('') για αυτόματο εντοπισμό
  ```
* **Τοπικές δοκιμές (Debug Mode):** Για τοπικές δοκιμές χωρίς σύνδεση στο CAS, ενεργοποιήστε το Debug mode στο `conf.php`:
  ```php
...
- `exp.php`: Παραγωγή και αυτόματη μετατροπή βεβαιώσεων σε PDF για μεμονωμένα προγράμματα.
- `download_all_vev.php`: Μαζική παραγωγή, ταχεία (batch) μετατροπή σε PDF και λήψη αρχείου ZIP με αυτόματο καθαρισμό προσωρινών αρχείων.
- `help_catalog.php`: Αυτόνομη σελίδα οδηγιών για τη δημοσίευση στον κατάλογο.
